...
def command_req_handler(cmd_obj: dict):
    ...
    elif cmd_obj['command'] == 'RGB':
        new_rgb_color = tuple(int(cmd_obj['args']['hex_color'][i:i + 2], 16) for i in (0, 2, 4))
        device_state = f"{new_rgb_color[0]} {new_rgb_color[1]} {new_rgb_color[2]}"
        print(f"Device new state: {device_state}")
        _mqtt_client.publish(f"state/{device_name}/main", device_state, retain=True)
...
_mqtt_client.subscribe(f"command/{device_name}/rgb_1")
